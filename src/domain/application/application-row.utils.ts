...
/**
 * Sort by applied date, then by id (ids are timestamp-prefixed via Date.now().toString(36)).
 * Same-day AI captures / form saves then stay newest-first when order is 'desc'.
 */
export function compareApplicationsByRecency(a: ApplicationRow, b: ApplicationRow, order: 'asc' | 'desc' = 'desc'): number {
  const timeA = Date.parse(a.date_applied) || 0;
  const timeB = Date.parse(b.date_applied) || 0;
  if (timeA !== timeB) {
...
export function mergeApplicationRow(existingRow: readonly string[], updates: Partial<ApplicationRow>): string[] {
